        ...
        def dfs(course) -> set[int]:
            if course not in preMap:
                for prereq in adj[course]:
                    preMap[course] |= dfs(prereq)
                # ultra important -> need to add itself, otherwise,
                # the children will not know where it came from
                # This is more clean that passing the prereq in preMap[course]
